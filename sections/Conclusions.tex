...
An annular shower shape representation coupled to multi-layer perceptron neural networks (the \rnn{}) provides an innovative machine-learning-based design for online electron selection based on calorimetry information in the ATLAS experiment, aiming to meet specific trigger system requirements, such as reducing fake electrons.

For this, a feature extraction algorithm was implemented to produce
concentric ring energy sums centered in the hottest cell of each calorimeter layer, which have been shown to provide an efficient description of calorimeter shower profiles and are fast computed through simple operations. 
...
In the second half of 2017, the \rnn{} algorithm replaced a cut-based strategy in the \fastcalo{} decision step of the \hlt{} and became the baseline algorithm for triggering events containing electrons with \ET above \SI{15}{\GeV}, as part of the ATLAS online system's CPU-time reduction campaign. Furthermore, it was the first time a neural-network method was employed as a baseline event-selection algorithm in the ATLAS trigger system.


During the 2017 operation, the \rnn{} algorithm, trained on simulated data, reduced fake-electron trigger rates by a factor of 13.75 in the \fastcalo{} and by a factor of 2 for the \hlt{} final decision. At the same time, the \rnn{} achieved high electron-selection efficiencies very similar to those of relevant single-electron triggers without the \rnn{}. For 2018, the \rnn{} was trained with collision data, resulting in an estimated additional fake-rate reduction factor of 1.2--1.7 for fake-electron triggers in the \fastcalo{} step with respect to 2017. 


The \rnn{} selection was also studied using offline likelihood identification as a reference to obtain additional insights into its behavior by using two equivalent single-electron triggers with and without the \rnn{}. 
In this analysis, a high level of agreement was observed between the two triggers. Finally, shower-shape and tracking variables used in the offline likelihood identification were compared for electron data collected with the \rnn{}-and cut-based triggers, and a strong statistical similarity was observed, indicating no bias in these variables introduced by the \rnn{} trigger configuration. This confirms that the \rnn{} trigger does not distort the inputs of the offline selection. As a result, the improved efficiency–rejection performance observed with the \rnn{} trigger can be attributed to its online behavior, without compromising the integrity of the offline identification



%Finally, the \enquote{homogeneity tests} between the data collected using triggers with and without the \rnn{} resulted in similar $p$-values. In other words, the fluctuations in the profiles of the calorimetry and track variables are mainly statistical, with no sign of systematic effects due to the trigger configuration.
